...
def mono_lingual_experiment(
    ...
    # valdiate model performance on test set
    ...
    torch.save(
        validate_target,
        RESULTS_DIR / f"raw_data/mono_{low_lang}_{sample_count}_{exp_id}_target.pt",
    )
    torch.save(
    ...
    model.save_pretrained(RESULTS_DIR / f"mono_model_{low_lang}_{exp_id}/")


def separate_few_shot_experiment(
    exp_id, base_lang, low_lang, model_name, sample_count, cased=True
):
    # get base lang data
    ...
    if os.path.isdir(RESULTS_DIR / f"model_multingual_bert_{base_lang}/"):
        # skip training the model on base lang if already trained
        model = BertForSequenceClassification.from_pretrained(
            RESULTS_DIR / f"model_multingual_bert_{base_lang}/"
        )
        ...
        loss_function = torch.nn.CrossEntropyLoss()
        optimizer = AdamW(
            model.parameters(), lr=LEARNING_RATE, weight_decay=DECAY_FACTOR
        )
        ...
        model.save_pretrained(
            RESULTS_DIR / f"model_multingual_bert_{base_lang}/"
        )  # save model trained on base language for future use

    # reinitate optimizer with inital learning rate
    optimizer = AdamW(
        model.parameters(), lr=LEARNING_RATE, weight_decay=DECAY_FACTOR
    )  # reinitialzie the optimizer
    for epoch in range(LOW_EPOCHS):
    ...
    model.save_pretrained(RESULTS_DIR / f"model_{exp_id}/")
